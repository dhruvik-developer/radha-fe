...
const generateDishCode = (price, sessionIndex) => {
  ...
  for (let i = 0; i < priceStr.length; i++) {
    ...
    if (i < priceStr.length - 1) {
       // if price is 4 digits or more, omit the dummy between the last two digits
...
const PDFViewPage = () => {
  ...
  const { itemData: rawItemData } = data || {};
  ...
  const { categorizedData: itemData, isCategorizing } =
    usePdfCategorizer(rawItemData);
  ...
  const shareOnWhatsApp = () => {
    ...
    const mobileNo = itemData?.mobile_no;
    shareToWhatsApp("pdf-content", fileName, mobileNo, toast);
  };

  // Generate Unique Key from Booking ID
  const uniqueKey = itemData
    ? itemData.id
  ...
  if (!itemData || isCategorizing) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen p-4 font-sans text-gray-800 bg-gradient-to-br from-[var(--color-primary-tint)] via-[var(--color-primary-tint)] to-[var(--color-primary-tint)]">
        {/* Action Bar (Top) */}
        <div className="flex justify-end gap-3 w-full mb-6 max-w-4xl no-print-button">
          <button
            className="flex items-center gap-2 px-6 py-3 bg-white text-gray-700 border border-gray-200 rounded-xl shadow-md hover:bg-gray-50 hover:text-[var(--color-primary)] hover:border-[var(--color-primary)]/30 focus:ring-4 focus:ring-[var(--color-primary-soft)] transition-all cursor-pointer font-semibold text-sm group"
          ...
          <button
            className="flex items-center gap-2 px-6 py-3 bg-[#25D366] text-white rounded-xl shadow-md hover:bg-[#20bd5a] focus:ring-4 focus:ring-green-200 transition-all cursor-pointer font-semibold text-sm group"
          ...
          <button
            className="flex items-center gap-2 px-6 py-3 bg-[var(--color-primary)] text-white rounded-xl shadow-md hover:bg-[var(--color-primary-text)] focus:ring-4 focus:ring-[var(--color-primary-soft)] transition-all cursor-pointer font-semibold text-sm group"
            onClick={downloadPDF}
            title="Download PDF"
          >
            <svg
                    ...
                    >
                      ...
                      <div className="bg-gradient-to-r from-[var(--color-primary-tint)] to-white px-5 py-3 border-b border-[var(--color-primary-border)]/30 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-4 text-xs font-bold text-gray-500">
                          ...
                          <span className="bg-[var(--color-primary-tint)] text-[var(--color-primary-text)] px-2.5 py-1 rounded-md tracking-wider">
                            ID: {generateDishCode(session.per_dish_amount, sIdx)}
                          </span>
                        </div>
                      </div>
              ...
              <div className="space-y-6">
                {itemData.sessions.map((session, sIdx) => (
                  ...
                  >
                    ...
                    {/* Waiter Service specific to this session */}
                    {session.waiter_service && session.waiter_service.type && (
                      <div className="px-5 py-4 bg-[var(--color-primary-tint)]/40 border-t border-[var(--color-primary-border)]/30">
                        <span className="text-xs font-bold text-[var(--color-primary-tint)]0 uppercase tracking-wider block mb-2">
                          Waiter Service
                        </span>
